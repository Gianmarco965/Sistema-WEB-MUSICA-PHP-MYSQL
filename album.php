...
$consulta_resultados = mysqli_query($db, "SELECT a.idalbum,a.nombrealbum,a.anio,a.valor,d.nomdisquera,CONCAT(r.nombreartista ,' ',r.apellidoartista) as 'artista',a.portada FROM album a inner join artista r on r.idartista=a.idartista inner join disquera d on d.iddisquera=a.iddisquera  LIMIT" .$empezar_desde.",". $xpagina."");
$sql1="SELECT a.idalbum,a.nombrealbum,a.anio,a.valor,d.nomdisquera,CONCAT(r.nombreartista ,' ',r.apellidoartista) as 'artista',a.portada FROM album a inner join artista r on r.idartista=a.idartista inner join disquera d on d.iddisquera=a.iddisquera LIMIT " .$empezar_desde.','. $xpagina."";
